...
public:
  ...
  void updata(int x, int p, int L, int R, pair<ll, ll> num) {
    ...
    int M = (L + R) / 2;
    if (p <= M)
      updata(x << 1, p, L, M, num);
    else
      updata(x << 1 | 1, p, M + 1, R, num);
  ...
  pair<ll, ll> query(int x, int l, int r, int L, int R) {
    ...
    if (l <= M)
      ans = min(ans, query(x << 1, l, r, L, M));
    if (r > M)
      ans = min(ans, query(x << 1 | 1, l, r, M + 1, R));
    return ans;
  }
};
struct Point {
...
//  https://csacademy.com/contest/ieeextreme-practice/task/parkour
